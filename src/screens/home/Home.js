...
const Home = (props) => {
    ...
    const filterApplyHandler = () => {
        let statusReleased = "?status=RELEASED";
        if (movieName !== "") {
            statusReleased += "&title=" + movieName;
        }
        if (genres.length > 0) {
            statusReleased += "&genres=" + genres.toString();
        }
        if (artists.length > 0) {
            statusReleased += "&artists=" + artists.toString();
        }
        if (releaseDateStart !== "") {
            statusReleased += "&start_date=" + releaseDateStart;
        }
        if (releaseDateEnd !== "") {
            statusReleased += "&end_date=" + releaseDateEnd;
        }

        let dataFilter = null;

        fetch(props.baseUrl + "movies" + statusReleased, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
                "Cache-Control": "no-cache"
